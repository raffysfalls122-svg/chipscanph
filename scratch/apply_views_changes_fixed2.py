...
# 1. Replace run_ocr_matching_task(img) using literal slice replacement
start_marker = "def run_ocr_matching_task(img):"
end_marker = "return ocr_text_to_save"

start_idx = code.find(start_marker)
if start_idx == -1:
    print("Error: run_ocr_matching_task not found!")
    sys.exit(1)

end_idx = code.find(end_marker, start_idx)
if end_idx == -1:
    print("Error: return ocr_text_to_save not found after run_ocr_matching_task!")
    sys.exit(1)
end_idx += len(end_marker)

optimized_ocr = """def run_ocr_matching_task(img):
...
code = code[:start_idx] + optimized_ocr + code[end_idx:]
print("run_ocr_matching_task replaced literal.")
...
code = code.replace(manual_err_block, manual_err_replacement)
print("manual check auto-submit approval replaced.")

# B. For image scan checks (after history_entry.image.save)
scan_err_block = """                # Save image attachment
                image_file.seek(0)
                history_entry.image.save(image_file.name, image_file, save=True)"""

scan_err_replacement = """                # Save image attachment
                image_file.seek(0)
                history_entry.image.save(image_file.name, image_file, save=True)
...
code = code.replace(scan_err_block, scan_err_replacement)
print("image scan auto-submit approval replaced.")
...
code = code.replace(api_chips_hdr, api_chips_replacement)
print("api_chips auth check replaced.")
...
code = code.replace(api_del_chip_hdr, api_del_chip_replacement)
print("api_delete_chip auth check replaced.")
...
code = code.replace(api_hist_clear_hdr, api_hist_clear_replacement)
print("api_history_clear auth check replaced.")
...
code = code.replace(api_users_hdr, api_users_replacement)
print("api_users auth check replaced.")
...
code = code.replace(api_del_user_hdr, api_del_user_replacement)
print("api_delete_user auth check replaced.")
...
# Write modified code back to views.py using windows newline="\\r\\n"
