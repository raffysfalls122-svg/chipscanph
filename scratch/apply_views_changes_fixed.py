...
# Normalize CRLF to LF for standard regex matching
code = code.replace("\r\n", "\n")

# 1. Replace run_ocr_matching_task(img)
# Using triple single-quotes for raw string to avoid escaping quotes
...
# Replace run_ocr_matching_task(img) pattern
...
# Define grade constants or import them if needed
# Wait, let's see if GRADE_BY_SIZE exists or needs to be defined
new_endpoints = '''
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'reject':
                ...
                grade = data.get('grade', 'A1')  # Standard fallback or passed grade
